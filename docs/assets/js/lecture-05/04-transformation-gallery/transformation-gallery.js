(() => {
  ...
  const presets = {
    identity: { matrix: [[1, 0], [0, 1]], title: 'identity — nothing changes' },
    stretch: { matrix: [[2, 0], [0, 1]], title: 'horizontal stretch — widths double' },
    shear: { matrix: [[1, 1], [0, 1]], title: 'shear — height produces sideways displacement' },
    reflection: { matrix: [[-1, 0], [0, 1]], title: 'reflection — left and right exchange places' },
    rotation: { matrix: [[0, -1], [1, 0]], title: 'rotation — every direction turns through 90°' }
  };

  const state = { matrix: presets.identity.matrix };
  ...
  for (let k = -5; k <= 5; k += 1) {
    board.create('segment', [[k, -5], [k, 5]], {
      strokeColor: '#dde2e8', strokeWidth: 1, dash: 2, ...fixed
    });
    board.create('segment', [[-5, k], [5, k]], {
      strokeColor: '#dde2e8', strokeWidth: 1, dash: 2, ...fixed
    });
    board.create('curve', [
      t => transform(k, t)[0], t => transform(k, t)[1], -5, 5
    ], { strokeColor: '#7fa9c8', strokeWidth: k === 0 ? 3 : 1.5, ...fixed });
    board.create('curve', [
      t => transform(t, k)[0], t => transform(t, k)[1], -5, 5
    ], { strokeColor: '#7fa9c8', strokeWidth: k === 0 ? 3 : 1.5, ...fixed });
  }
  ...
  function setMode(mode) {
    ...
    window.LectureMath?.set(matrixReadout, window.LectureMath.matrix('A', state.matrix));
    board.fullUpdate();
  }
