(() => {
  ...
  /* Only two input lines, two output lines and one short direction sample per family. */
  const parallelBases = [[-2.5, -1.5], [-1.5, -0.5]];
  ...
  const vStart = [-4.15, -2.15];
  const vEnd = add(vStart, scale(0.42, v));
  arrow('parallel', vStart, vEnd, { strokeColor: '#8d969f', strokeWidth: 3.2 });
  text('parallel', add(vEnd, [0.08, 0.08]), 'v', { color: '#6f7882' });

  const AvStart = [0.55, -1.85];
  const AvEnd = add(AvStart, scale(0.34, Av));
  arrow('parallel', AvStart, AvEnd, { strokeColor: '#2f6f9f', strokeWidth: 3.2 });
  text('parallel', add(AvEnd, [0.08, 0.08]), 'Av', { color: '#2f6f9f' });
